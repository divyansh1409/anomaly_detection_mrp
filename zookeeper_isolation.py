#!/usr/bin/env python3
"""
Zookeeper Isolation Forest Anomaly Detection

Isolation Forest for Zookeeper logs. Unsupervised anomaly detection.
...
class ZookeeperIsolationForest:
    def __init__(self, data_path="Zookeeper_features.csv"):
        """Initialize the Zookeeper Isolation Forest detector."""
        ...
        os.makedirs("zookeeper_isolation_results", exist_ok=True)
    ...
    def train_model(self, contamination=0.01, n_estimators=100, max_samples='auto'):  # Low contamination for Zookeeper
    ...
    def plot_results(self):
        ...
        plt.tight_layout()
        plt.savefig('zookeeper_isolation_results/isolation_forest_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_detailed_analysis(self):
        ...
        plt.tight_layout()
        plt.savefig('zookeeper_isolation_results/detailed_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def save_results(self):
        ...
        # Save model
        joblib.dump(self.model, 'zookeeper_isolation_results/isolation_forest_model.pkl')
        
        # Save scaler
        joblib.dump(self.scaler, 'zookeeper_isolation_results/scaler.pkl')
        ...
        summary_df.to_csv('zookeeper_isolation_results/model_summary.csv', index=False)
        
        # Save detailed results
        with open('zookeeper_isolation_results/detailed_results.txt', 'w') as f:
            f.write("Zookeeper Isolation Forest Anomaly Detection Results\n")
            f.write("=" * 60 + "\n\n")
        ...
        logger.info("Results saved to zookeeper_isolation_results/ directory")
    
    def run_analysis(self, contamination=0.01, n_estimators=100, max_samples='auto'):
        """Run the complete Isolation Forest analysis."""
        logger.info("Starting Zookeeper Isolation Forest Analysis")
        ...
        logger.info("Zookeeper Isolation Forest analysis completed successfully!")
        
        return self.evaluation_results

def main():
    """Main function to run the analysis."""
    detector = ZookeeperIsolationForest()
    results = detector.run_analysis()
    
    # Print summary
    print("\n" + "="*60)
    print("ZOOKEEPER ISOLATION FOREST RESULTS SUMMARY")
    print("="*60)
    ...
    print(f"\nResults saved in: zookeeper_isolation_results/")
