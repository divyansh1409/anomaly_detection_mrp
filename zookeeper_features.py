...
def extract_zookeeper_features():
    print("Extracting features for Zookeeper dataset...")
    df = pd.read_csv('Zookeeper_2k.log_structured.csv')
    print(f"Loaded {len(df)} Zookeeper log entries")
    ...
    # 2. Zookeeper-Specific Patterns (based on actual dataset analysis)
    features['contains_quorum'] = df['Content'].str.contains(r'\bQuorum\b', case=False).astype(int)
    features['contains_zookeeper'] = df['Content'].str.contains(r'\bZooKeeper\b', case=False).astype(int)
    features['contains_leader'] = df['Content'].str.contains(r'\bLeader\b', case=False).astype(int)
    features['contains_follower'] = df['Content'].str.contains(r'\bFollower\b', case=False).astype(int)
    features['contains_election'] = df['Content'].str.contains(r'\bElection\b', case=False).astype(int)
    features['contains_connection'] = df['Content'].str.contains(r'\bconnection\b', case=False).astype(int)
    features['contains_notification'] = df['Content'].str.contains(r'\bNotification\b', case=False).astype(int)
    features['contains_timeout'] = df['Content'].str.contains(r'\btime out\b', case=False).astype(int)
    features['contains_received'] = df['Content'].str.contains(r'\bReceived\b', case=False).astype(int)
    features['contains_send'] = df['Content'].str.contains(r'\bSend\b', case=False).astype(int)
    features['contains_worker'] = df['Content'].str.contains(r'\bworker\b', case=False).astype(int)
    features['contains_listener'] = df['Content'].str.contains(r'\bListener\b', case=False).astype(int)
    features['contains_handler'] = df['Content'].str.contains(r'\bHandler\b', case=False).astype(int)
    features['contains_peer'] = df['Content'].str.contains(r'\bPeer\b', case=False).astype(int)
    features['contains_server'] = df['Content'].str.contains(r'\bServer\b', case=False).astype(int)
    features['contains_factory'] = df['Content'].str.contains(r'\bFactory\b', case=False).astype(int)
    
    # 3. Zookeeper-Specific Error Patterns
    ...
    features['contains_interrupted'] = df['Content'].str.contains(r'\bInterrupted\b', case=False).astype(int)
    features['contains_leaving'] = df['Content'].str.contains(r'\bleaving\b', case=False).astype(int)
    features['contains_terminating'] = df['Content'].str.contains(r'\bterminating\b', case=False).astype(int)
    
    # 4. Zookeeper-Specific System Patterns
    ...
    features['contains_process_id'] = df['Content'].str.contains(r'\b\d+:\b', case=False).astype(int)
    features['contains_thread'] = df['Content'].str.contains(r'\bthread\b', case=False).astype(int)
    features['contains_queue'] = df['Content'].str.contains(r'\bqueue\b', case=False).astype(int)
    features['contains_message'] = df['Content'].str.contains(r'\bmessage\b', case=False).astype(int)
    features['contains_request'] = df['Content'].str.contains(r'\brequest\b', case=False).astype(int)
    features['contains_response'] = df['Content'].str.contains(r'\bresponse\b', case=False).astype(int)
    
    # 5. Numeric Patterns (specific to Zookeeper)
    ...
    features['process_id_count'] = df['Content'].str.count(r'\b\d+:\b')
    features['timeout_value_count'] = df['Content'].str.count(r'\d+\s*$')
    features['hex_number_count'] = df['Content'].str.count(r'0x[0-9a-fA-F]+')
    ...
    # Add component-based features (only the ones that exist in Zookeeper)
    ...
    # Note: Zookeeper doesn't have Level column, so skip level dummies
    ...
    # Force all event and component columns to int
    for col in feature_df.columns:
        if col.startswith('event_') or col.startswith('component_'):
            feature_df[col] = pd.to_numeric(feature_df[col], errors='coerce').fillna(0).astype(int)
    
    # Save features
    feature_df.to_csv('Zookeeper_features.csv', index=False)
    print(f"Extracted {len(feature_df.columns)} features for Zookeeper dataset")
    print(f"Features saved to Zookeeper_features.csv")
...
if __name__ == "__main__":
    zookeeper_features = extract_zookeeper_features() 