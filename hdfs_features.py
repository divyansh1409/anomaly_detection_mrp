...
def extract_hdfs_features():
    print("Extracting features for HDFS dataset...")
    df = pd.read_csv('HDFS_2k.log_structured.csv')
    print(f"Loaded {len(df)} HDFS log entries")
    ...
    # 2. HDFS-Specific Patterns (based on actual dataset analysis)
    features['contains_packet_responder'] = df['Content'].str.contains(r'\bPacketResponder\b', case=False).astype(int)
    features['contains_block'] = df['Content'].str.contains(r'\bblock\b', case=False).astype(int)
    features['contains_blk_id'] = df['Content'].str.contains(r'blk_[-]?\d+', case=False).astype(int)
    features['contains_datanode'] = df['Content'].str.contains(r'\bDataNode\b', case=False).astype(int)
    features['contains_namesystem'] = df['Content'].str.contains(r'\bNameSystem\b', case=False).astype(int)
    features['contains_terminating'] = df['Content'].str.contains(r'\bterminating\b', case=False).astype(int)
    features['contains_added'] = df['Content'].str.contains(r'\bis added to\b', case=False).astype(int)
    features['contains_size'] = df['Content'].str.contains(r'\bsize\s+\d+\b', case=False).astype(int)
    features['contains_received'] = df['Content'].str.contains(r'\bReceived\b', case=False).astype(int)
    features['contains_updated'] = df['Content'].str.contains(r'\bupdated\b', case=False).astype(int)
    
    # 3. HDFS-Specific Error Patterns
    ...
    # 4. HDFS-Specific System Patterns
    ...
    features['contains_block_id'] = df['Content'].str.contains(r'blk_[-]?\d+', case=False).astype(int)
    
    # 5. Numeric Patterns (specific to HDFS)
    ...
    features['block_id_count'] = df['Content'].str.count(r'blk_[-]?\d+')
    features['size_value_count'] = df['Content'].str.count(r'size\s+\d+')
    ...
    # Add component-based features (only the ones that exist in HDFS)
    ...
    # Save features
    feature_df.to_csv('HDFS_features.csv', index=False)
    print(f"Extracted {len(feature_df.columns)} features for HDFS dataset")
    print(f"Features saved to HDFS_features.csv")
...
if __name__ == "__main__":
    hdfs_features = extract_hdfs_features() 