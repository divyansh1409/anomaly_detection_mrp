#!/usr/bin/env python3
"""
HDFS Isolation Forest Anomaly Detection

Isolation Forest for HDFS logs. Unsupervised anomaly detection.
...
class HDFSIsolationForest:
    def __init__(self, data_path="HDFS_features.csv"):
        """Initialize the HDFS Isolation Forest detector."""
        ...
        os.makedirs("hdfs_isolation_results", exist_ok=True)
    ...
    def train_model(self, contamination=0.1, n_estimators=100, max_samples='auto'):
    ...
    def plot_results(self):
        ...
        plt.tight_layout()
        plt.savefig('hdfs_isolation_results/isolation_forest_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_detailed_analysis(self):
        ...
        plt.tight_layout()
        plt.savefig('hdfs_isolation_results/detailed_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def save_results(self):
        ...
        # Save model
        joblib.dump(self.model, 'hdfs_isolation_results/isolation_forest_model.pkl')
        
        # Save scaler
        joblib.dump(self.scaler, 'hdfs_isolation_results/scaler.pkl')
        ...
        summary_df.to_csv('hdfs_isolation_results/model_summary.csv', index=False)
        
        # Save detailed results
        with open('hdfs_isolation_results/detailed_results.txt', 'w') as f:
            f.write("HDFS Isolation Forest Anomaly Detection Results\n")
            f.write("=" * 60 + "\n\n")
        ...
        logger.info("Results saved to hdfs_isolation_results/ directory")
    
    def run_analysis(self, contamination=0.1, n_estimators=100, max_samples='auto'):
        """Run the complete Isolation Forest analysis."""
        logger.info("Starting HDFS Isolation Forest Analysis")
        ...
        logger.info("HDFS Isolation Forest analysis completed successfully!")
        
        return self.evaluation_results

def main():
    """Main function to run the analysis."""
    detector = HDFSIsolationForest()
    results = detector.run_analysis()
    
    # Print summary
    print("\n" + "="*60)
    print("HDFS ISOLATION FOREST RESULTS SUMMARY")
    print("="*60)
    ...
    print(f"\nResults saved in: hdfs_isolation_results/")
