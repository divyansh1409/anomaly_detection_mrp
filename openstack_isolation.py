#!/usr/bin/env python3
"""
OpenStack Isolation Forest Anomaly Detection

Isolation Forest for OpenStack logs. Unsupervised anomaly detection.
...
class OpenStackIsolationForest:
    def __init__(self, data_path="OpenStack_features.csv"):
        """Initialize the OpenStack Isolation Forest detector."""
        ...
        os.makedirs("openstack_isolation_results", exist_ok=True)
    ...
    def train_model(self, contamination=0.01, n_estimators=100, max_samples='auto'):  # Low contamination for OpenStack
    ...
    def plot_results(self):
        ...
        # 2. ROC Curve (if labels available)
        if self.y is not None:
            plt.subplot(2, 3, 2)
        ...
        plt.tight_layout()
        plt.savefig('openstack_isolation_results/isolation_forest_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_detailed_analysis(self):
        ...
        plt.tight_layout()
        plt.savefig('openstack_isolation_results/detailed_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def save_results(self):
        ...
        # Save model
        joblib.dump(self.model, 'openstack_isolation_results/isolation_forest_model.pkl')
        
        # Save scaler
        joblib.dump(self.scaler, 'openstack_isolation_results/scaler.pkl')
        ...
        summary_df.to_csv('openstack_isolation_results/model_summary.csv', index=False)
        
        # Save detailed results
        with open('openstack_isolation_results/detailed_results.txt', 'w') as f:
            f.write("OpenStack Isolation Forest Anomaly Detection Results\n")
            f.write("=" * 60 + "\n\n")
        ...
        logger.info("Results saved to openstack_isolation_results/ directory")
    
    def run_analysis(self, contamination=0.01, n_estimators=100, max_samples='auto'):
        """Run the complete Isolation Forest analysis."""
        logger.info("Starting OpenStack Isolation Forest Analysis")
        ...
        logger.info("OpenStack Isolation Forest analysis completed successfully!")
        
        return self.evaluation_results

def main():
    """Main function to run the analysis."""
    detector = OpenStackIsolationForest()
    results = detector.run_analysis()
    
    # Print summary
    print("\n" + "="*60)
    print("OPENSTACK ISOLATION FOREST RESULTS SUMMARY")
    print("="*60)
    ...
    print(f"\nResults saved in: openstack_isolation_results/")
