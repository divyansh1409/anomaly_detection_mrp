...
def extract_thunderbird_features():
    print("Extracting features for Thunderbird dataset...")
    df = pd.read_csv('Thunderbird_2k.log_structured.csv')
    print(f"Loaded {len(df)} Thunderbird log entries")
    ...
    # 2. Thunderbird-Specific Patterns (based on actual dataset analysis)
    features['contains_crond'] = df['Content'].str.contains(r'\bcrond\b', case=False).astype(int)
    features['contains_sshd'] = df['Content'].str.contains(r'\bsshd\b', case=False).astype(int)
    features['contains_sendmail'] = df['Content'].str.contains(r'\bsendmail\b', case=False).astype(int)
    features['contains_dhcpd'] = df['Content'].str.contains(r'\bdhcpd\b', case=False).astype(int)
    features['contains_xinetd'] = df['Content'].str.contains(r'\bxinetd\b', case=False).astype(int)
    features['contains_snmpd'] = df['Content'].str.contains(r'\bsnmpd\b', case=False).astype(int)
    features['contains_acpi'] = df['Content'].str.contains(r'\bACPI\b', case=False).astype(int)
    features['contains_cpu'] = df['Content'].str.contains(r'\bCPU\b', case=False).astype(int)
    features['contains_console'] = df['Content'].str.contains(r'\bConsole\b', case=False).astype(int)
    features['contains_ext3'] = df['Content'].str.contains(r'\bEXT3-fs\b', case=False).astype(int)
    features['contains_ioapic'] = df['Content'].str.contains(r'\bIOAPIC\b', case=False).astype(int)
    features['contains_ip'] = df['Content'].str.contains(r'\bIP\b', case=False).astype(int)
    features['contains_pci'] = df['Content'].str.contains(r'\bPCI\b', case=False).astype(int)
    features['contains_selinux'] = df['Content'].str.contains(r'\bSELinux\b', case=False).astype(int)
    features['contains_tcp'] = df['Content'].str.contains(r'\bTCP\b', case=False).astype(int)
    features['contains_vfs'] = df['Content'].str.contains(r'\bVFS\b', case=False).astype(int)
    
    # 3. Thunderbird-Specific Error Patterns
    ...
    features['contains_fatal'] = df['Content'].str.contains(r'\b(fatal|Fatal|FATAL)\b', case=False).astype(int)
    
    # 4. Thunderbird-Specific System Patterns
    ...
    features['contains_audit'] = df['Content'].str.contains(r'\baudit\b', case=False).astype(int)
    features['contains_device'] = df['Content'].str.contains(r'\bdevice\b', case=False).astype(int)
    features['contains_driver'] = df['Content'].str.contains(r'\bdriver\b', case=False).astype(int)
    features['contains_kernel'] = df['Content'].str.contains(r'\bkernel\b', case=False).astype(int)
    features['contains_hardware'] = df['Content'].str.contains(r'\bhardware\b', case=False).astype(int)
    features['contains_network'] = df['Content'].str.contains(r'\bnetwork\b', case=False).astype(int)
    features['contains_security'] = df['Content'].str.contains(r'\bsecurity\b', case=False).astype(int)
    features['contains_detected'] = df['Content'].str.contains(r'\bdetected\b', case=False).astype(int)
    
    # 5. Numeric Patterns (specific to Thunderbird)
    ...
    features['process_id_count'] = df['Content'].str.count(r'\b\d+:\d+\b')
    features['hardware_id_count'] = df['Content'].str.count(r'[A-Z]{2,4}\d+')
    ...
    # Add component-based features (only the ones that exist in Thunderbird)
    ...
    # Note: Thunderbird doesn't have Level column, so skip level dummies
    ...
    # Save features
    feature_df.to_csv('Thunderbird_features.csv', index=False)
    print(f"Extracted {len(feature_df.columns)} features for Thunderbird dataset")
    print(f"Features saved to Thunderbird_features.csv")
...
if __name__ == "__main__":
    thunderbird_features = extract_thunderbird_features() 